...
class CaT(nn.Module):
	...
	def __init__(self, dropout_rate, num_heads, head_size, n_layers, dag, device, pred_column=None, continuous_outcome=True):
		...
		self.loss_func = torch.nn.MSELoss() if continuous_outcome else torch.nn.BCELoss()
	...
	def forward(self, X, targets=None):
		...
		X = self.lm_head(X)
		X = X[:, -1, 0] if self.continuous_outcome else torch.sigmoid(X[:, -1, 0])  # TODO: predict all 'tokens'

		# MSE, L1, for continuous, BCE + logits/sigmoid ,  Multiclass Cross-entropy
